...
class AppState extends ChangeNotifier {
  ...
  Future<void> removeRouter(String id) async {
    ...
    final router = _routerService!.routers.firstWhere(
      (r) => r.id == id,
      orElse: () => throw Exception('Роутер не найден'),
    );
  ...
  Future<bool> login(
    ...
    try {
      ...
      if (_authService!.isAuthenticated) {
        ...
        _errorMessage =
            'Вход не выполнен: неверные учётные данные или хост недоступен.';
        _isLoading = false;
        notifyListeners();
        return false;
      }
  ...
  Future<void> fetchDashboardData() async {
    if (_reviewerModeEnabled) {
      ...
      try {
        ...
        _dashboardError = 'Не удалось получить данные панели: $e';
        _isDashboardLoading = false;
        notifyListeners();
      }
      return;
    ...
    try {
      ...
      if (errorMessage.contains('Access denied')) {
        _dashboardError = 'Доступ запрещён: проверьте права RPC для этого пользователя.';
      } else {
        _dashboardError = 'Не удалось получить данные панели: $e';
      }
  ...
  Future<bool> setWirelessRadioState(
    ...
    try {
      ...
      _dashboardError = 'Не удалось переключить Wi-Fi: $e';
      notifyListeners();
      return false;
    }
  }
