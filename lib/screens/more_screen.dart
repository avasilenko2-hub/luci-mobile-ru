...
class _MoreScreenState extends ConsumerState<MoreScreen> {
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Роутер снова в сети, переподключение…',
    ...
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('Вы уверены, что хотите перезагрузить роутер?'),
          actions: <Widget>[
            TextButton(
              child: const Text('Отмена'),
              onPressed: () {
              ...
              onPressed: () async {
                  ...
                  SnackBar(
                    content: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Перезагрузка… Соединение будет прервано.',
                            style: theme.textTheme.bodyMedium?.copyWith(
                              color: colorScheme.onPrimary,
                            ),
                          ),
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(
                      success
                          ? 'Команда перезагрузки успешно отправлена.'
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const Text('Мобильный клиент для роутеров OpenWrt.'),
                const SizedBox(height: 16),
                const Text('Открытый исходный код, бесплатно.'),
                const SizedBox(height: 16),
                InkWell(
                  onTap: () async {
                    ...
                    if (!success && context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: const Text('Не удалось открыть репозиторий'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const LuciAppBar(title: 'Ещё'),
      body: SingleChildScrollView(
        padding: const EdgeInsets.symmetric(vertical: LuciSpacing.sm),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const LuciSectionHeader('Управление устройством'),
            Builder(
              builder: (context) {
                ...
                return _MoreScreenSection(
                  tiles: [
                    _buildMoreTile(
                      ...
                      subtitle: 'Выполнить перезапуск системы',
                      onTap: isRebooting
                          ? null
            ...
            const LuciSectionHeader('Приложение'),
            _MoreScreenSection(
              tiles: [
                _buildMoreTile(
                  context,
                  icon: Icons.router,
                  iconColor: Theme.of(context).colorScheme.primary,
                  title: 'Управление роутерами',
                  subtitle: 'Изменить или удалить сохранённые роутеры',
                  onTap: () {
                ...
                _buildMoreTile(
                  ...
                  iconColor: Theme.of(context).colorScheme.primary,
                  title: 'Настройки',
                  subtitle: 'Настроить параметры приложения',
                  onTap: () {
                    Navigator.of(context).push(
                      MaterialPageRoute(
                ...
                _buildMoreTile(
                  ...
                  title: 'О приложении',
                  subtitle: 'Версия и сведения о приложении',
                ...
                _buildMoreTile(
                  ...
                  title: 'Выйти',
                  subtitle: 'Завершить сеанс и выйти',
