...
class _InterfacesScreenState extends ConsumerState<InterfacesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const LuciAppBar(title: 'Интерфейсы'),
      body: SafeArea(
        top: true,
        bottom: false,
        child: Stack(
                ...
                builder: (context) {
                  ...
                  if (dashboardError != null && dashboardData == null) {
                    return LuciErrorDisplay(
                      title: 'Не удалось загрузить интерфейсы',
                      message:
                          'Не удалось подключиться к роутеру. Проверьте сетевое подключение и настройки роутера.',
                  ...
                  if (dashboardData == null) {
                    return LuciEmptyState(
                      title: 'Нет данных об интерфейсах',
                      message:
                          'Не удалось получить информацию об интерфейсах. Потяните вниз для обновления или нажмите кнопку ниже.',
                  ...
                  return CustomScrollView(
                    ...
                    slivers: [
                      SliverToBoxAdapter(child: LuciSectionHeader('Проводные')),
                      _buildWiredInterfacesList(),
                      SliverToBoxAdapter(child: LuciSectionHeader('Беспроводные')),
  ...
  Widget _buildWirelessInterfacesList() {
          ...
          for (final iface in interfaces) {
            ...
            interfacesList.add({
              'name': _uciString(config['ssid']).isNotEmpty
                  ? _uciString(config['ssid'])
                  : (iwinfo['ssid']?.toString() ?? 'Без имени'),
              'subtitle':
                  '$mode • Кан. ${iwinfo['channel']?.toString() ?? _uciString(config['channel'], 'N/A')}',
              'isEnabled': isEnabled,
              'deviceName': deviceName,
              'radioName': radioName,
              'ssid': ssid,
              'interfaceName': name,
              'details': {
                'Устройство': _uciString(config['device'], radioName),
                'Режим': _uciString(config['mode']).isNotEmpty
                    ? _uciString(config['mode'])
                    : (iwinfo['mode']?.toString() ?? 'N/A'),
                'Канал':
                    iwinfo['channel']?.toString() ??
                    _uciString(config['channel'], 'N/A'),
                'Сигнал': '${iwinfo['signal']?.toString() ?? '--'} dBm',
                'Сеть': (config['network'] is List)
    ...
    uciInterfaces.forEach((uciName, config) {
      if (!runtimeInterfaces.contains(uciName)) {
        ...
        final name = _uciString(config['ssid'], 'Без имени');
        interfacesList.add({
          'name': name,
          'subtitle':
          ...
          'interfaceName': name,
          'details': {
            'Устройство': radioName,
            'Режим': _uciString(config['mode'], 'N/A'),
  ...
  Widget _buildWiredDetails(BuildContext context, NetworkInterface interface) {
    return Column(
      children: [
        _buildDetailRow(context, 'Устройство', interface.device),
        _buildDetailRow(context, 'Время работы', interface.formattedUptime),
        if (interface.ipAddress != null)
          _buildDetailRow(
            ...
            interface.ipAddress!,
            onTap: () =>
                _copyToClipboard(context, interface.ipAddress!, 'IP-адрес'),
          ),
        if (interface.ipv6Addresses != null &&
          ...
          ...interface.ipv6Addresses!.map(
            (ipv6) => _buildDetailRow(
              ...
              'IPv6-адрес',
              ipv6,
              onTap: () => _copyToClipboard(context, ipv6, 'IPv6-адрес'),
            ),
          ),
        if (interface.gateway != null)
          _buildDetailRow(
            ...
            interface.gateway!,
            onTap: () =>
                _copyToClipboard(context, interface.gateway!, 'IP шлюза'),
          ),
  ...
  Widget _buildCohesivePeerRow(
    ...
    final publicKey = peer['public_key'] as String? ?? 'Неизвестно';
    ...
    String formatHandshakeTime(int timestamp) {
      ...
      if (difference.inSeconds < 0) return 'Никогда';
  ...
  void _copyToClipboard(BuildContext context, String text, String label) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('$label скопировано в буфер обмена'),
                        ...
                        Container(
                          ...
                          child: AnimatedScale(
                            ...
                            child: Icon(
                              ...
                              size: 22,
                              semanticLabel: 'Значок интерфейса',
                            ),
                          ),
                        ),
                        Positioned(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            ...
                            semanticsLabel: 'Имя интерфейса: ${widget.name}',
                          ),
                          ...
                          Text(
                            ...
                            semanticsLabel:
                                'Сведения об интерфейсе: ${widget.subtitle}',
                          ),
                        ],
                      ),
                    ),
