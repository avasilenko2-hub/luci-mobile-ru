...
class _RouterDashboardSettingsScreenState
  ...
  Future<void> _loadPreferences() async {
    try {
      ...
      if (appState.dashboardData == null) {
        setState(() {
          _errorMessage =
              'Не удалось загрузить данные панели. Проверьте подключение.';
          _isLoading = false;
        });
        return;
      }
      ...
      setState(() {
        _errorMessage = 'Не удалось загрузить настройки: $e';
        _isLoading = false;
      });
    }
  }
  ...
  Widget _buildThroughputSection() {
    ...
    return _buildSection(
      ...
      subtitle: 'Выберите интерфейсы для мониторинга',
      icon: Icons.speed,
  ...
  Widget _buildWirelessInterfacesSection() {
    ...
    return _buildSection(
      ...
      subtitle: 'Выберите, какие беспроводные сети показывать',
      icon: Icons.wifi,
      children: [
        Container(
          decoration: BoxDecoration(
  ...
  Widget _buildWiredInterfacesSection() {
    ...
    return _buildSection(
      ...
      subtitle: 'Выберите, какие проводные/VPN-интерфейсы показывать',
      icon: Icons.cable,
      children: [
        Container(
          decoration: BoxDecoration(
  ...
  Widget? _getInterfaceDescription(String interface) {
    ...
    if (lower.startsWith('wan')) {
      return Text('Глобальная сеть (WAN)',
          style: LuciTextStyles.cardSubtitle(context));
    } else if (lower.startsWith('lan')) {
      return Text('Локальная сеть (LAN)',
          style: LuciTextStyles.cardSubtitle(context));
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return const Scaffold(
        appBar: LuciAppBar(title: 'Настройки панели', showBack: true),
        body: Center(child: CircularProgressIndicator()),
      );
    }
    if (_errorMessage != null) {
      return Scaffold(
        appBar: const LuciAppBar(title: 'Настройки панели', showBack: true),
